...
function handleValidation(event) {
  ...
  axios.post("http://localhost:8080/school/login", values)
  .then((response) => {
    // console.log("User logged in successfully", response.data);
  
console.log("registrationsuccessfully"+response.data);
navigate("/volunteer");
  ...
  <p>or login with</p>
        ...
        <Icon icon="skill-icons:instagram" className="icons" onClick={handleInstagramLogin} />
          ...
          <div className="d-flex  justify-content-center align-items-center m-2">
            ...
            <Link to={'/school/signup'}> <a className="button" style={{ color: "#1877F2" }}>Register now</a></Link>
          </div>
        </div>
      </div>
    </div>
